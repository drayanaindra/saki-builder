...
Canonical schema lives in the `/saki-builder:prd` skill (`skill://prd`) — invoke it when available. This template mirrors that schema for use when the skill isn't reachable (e.g., subagent context). Both must stay in sync.
...
- Dir: `~/.omp/agent/agent-memory/senior-pm/`
- Protocol: `skill://saki-builder-runtime/docs/agent-memory-protocol.md` (load on demand when saving/recalling)
