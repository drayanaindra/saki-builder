...
> runtime dependency — `/reflect` and `/retro` write there regardless of
...
- **Non-interactive / PRD-driven** — `$ARGUMENTS` contains a path to a PRD (e.g.
  ...
  >    (`reviewer` + `qa` are what `/build` invokes).
  ...
  > product roadmap (Step 11b). The operator can run `/init-env` interactively later to
  > add these.
...
1c. **Choose + record the design engine** (INTERACTIVE mode only — SKIP in headless/PRD-driven mode):
    `/proto` can preview a PRD two ways — **native** (render the project's real design system
    into an HTML gallery; the canonical path `/build` reads) or **figma** (use a connected
    ...
    record lands at `.claude/design-engine.json`; `/proto` Step 0 reads it and routes.
...
3. **Create docs/project-context.md** — the ONE hand-written context file, scoped to what no tool derives:
   ...
   - **Do NOT write** god nodes, communities, per-file descriptions, module LOC, architecture stage, or
     business narrative — `graphify-out/GRAPH_REPORT.md`, `/arch-check` and the roadmap/PRDs
   ...
   - `/wrap` Phase 2a refreshes this file when a diff adds a deployable, a cross-boundary
...
7. **Create the `qa` agent**:
   ...
   - This agent is invoked programmatically (not by user via /qa)
   ...
   **Where Steps 5–7 write, and in what shape:**
   ...
   - **`claude`** → `.claude/agents/<name>.md`, frontmatter as described above (`tools:` as a CSV
     string, `model: sonnet|opus`, optional `color:`).
   - **`opencode`** → `.opencode/agent/<name>.md`, with three mandatory frontmatter differences —
     ...
     3. qualify the model as `provider/model` (e.g. `anthropic/claude-sonnet-4-6`), and map any named
...
11. **Initialize memory** (two files, two roles — mirror the global `~/.claude/memory/` split). These
   ...
   - Create `.claude/memory/patterns.md` (empty template) — the **promoted store**. `/reflect` writes
     ...
     auto-load into `/prd` / `/rplan` / `/build`. Seed it with a
...
11b. **Offer the product roadmap** (INTERACTIVE mode only — SKIP in headless/PRD-driven mode):
    ...
    `Set up a product roadmap now? It's how work gets started — /add categorizes each item
    (epic · feature · improvement · bug) and routes it. (y/n)`
    - **y** → scaffold `tasks/roadmap.md` via `/roadmap init` (ask for the product name, default
      the repo name), then offer to add the first 1–3 items with `/add`. Don't force it — one
      item is enough to demonstrate the flow; the rest can be added later.
    - **n** → skip; note the operator can run `/roadmap init` + `/add` anytime. Do not
      block init on this.
