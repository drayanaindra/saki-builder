---
description: "Autonomously execute a finished PRD (PRD-track) OR a single plan-track item (Improvement/Bug) end-to-end. PRD mode reads the PRD's vertical slices and runs /rplan → (/rplan-review if needed) → /approved → /qa → /reviewer → (security audit on security-relevant slices) on each slice; PLAN mode runs the same chain ONCE over one plan-track item — the hands-off equivalent of running /rplan → /rplan-review → /approved → /qa → /reviewer → /wrap by hand. Always runs the e2e suite and converges to clean before declaring done. No confirmation prompts. Usage — /build <E<n>|F<n>|prd-file.md | I<n>|B<n>|plan-file.md>."
---

# Autonomous PRD Executor

You are operating in **TRUST MODE** — fully autonomous execution. The PRD has already
been written, reviewed (via `/prd` → `/prd-review`), and **Locked** (via
`/proto`, Gate 1.5), and the user has pre-authorized this flow. Your single goal: **ship every
...
```
/rplan  →  /rplan-review (only if needed)  →  /approved  →  /qa  →  /reviewer  →  security audit (security-relevant slices only)
```

…then verifying the whole thing end-to-end and converging to clean (`/wrap --heal`).
...
- **Do NOT wait for plan approval.** `/rplan` normally stops for a human; here you
  auto-approve any plan whose **Blocking Set is empty** and proceed to `/approved` yourself.
- Agent-based sub-skills that are part of this flow (`/rplan-review`, `/reviewer`,
...
- **Completion signal.** You are done ONLY when every slice in the PRD is green — **or, in PLAN mode, the
  single plan-track item is green** — (`/qa` passes, `/reviewer` is clean, **and**
  any security audit a security-relevant slice/item required is clean), the e2e suite passes, **and** FINAL
  GATE 2 (`/wrap --heal`) has converged the tree to a clean `main`. At that point, and only
...
Usage: `/build <E<n>|F<n> | prd-file.md | I<n>|B<n> | plan-file.md>` (filler words are fine, e.g. `/build start build prd-wave-2.md`).

`/build` runs in one of **two modes**, decided by the argument (see *Mode detection* below):
...
`<id>` has no Child PRD yet (its value is `—`), **STOP**: `<id> has no PRD yet — run /pickup <id> first`.
...
> `/build tasks/prd-<slug>.md`) invokes this skill with the **PRD path**, not `<id>`. Do NOT
...
(1) its `**Child plan:**` link (written by `/rplan` Step 0.6 when it's set — the clean primary);
(2) fallback — grep `tasks/*-plan.md` headers for `**Item:** <id>` (the stamp `/rplan` always
...
stop — **PLAN mode runs `/rplan` itself** to create the plan (see The Single-Plan Loop, step
P1). This differs from the PRD `no-PRD` stop on purpose: the roadmap **item** (created by `/add`)
is the pre-existing scope unit here — the analogue of the PRD — and plan-track has no human lock gate, so a
missing plan is something `/build` *creates*, exactly as PRD mode runs `/rplan`
...
`/rplan` (the plan already exists — resume from review/approved). If the plan header carries
...
| id not found on `tasks/roadmap.md` | — | **STOP**: `<id> not found on roadmap — /add it, or run /rplan on a plan file` |
...
(`/wrap --heal`) run **identically** in both modes.
...
> extract its **Success Criteria** (→ the `/qa` targets) and the files/screens it touches
> (its Frontend checklist + any `tasks/<slug>-flow.md`). If the item has **no plan yet**, there is nothing
> to load here — The Single-Plan Loop step P1 creates it via `/rplan`. Then go straight to
...
### Reuse the `/proto` preview when one exists (verified at step 3.5)

If `tasks/proto-<slug>/notes.md` exists (the user ran `/proto` first), read it. It records the
...
> never locked (there is no `/proto` freeze for Improvement/Bug — that's the whole reason
> plan-track skips proto). Proceed to GATE 0 → GATE 2 → The Single-Plan Loop. The rest of this gate is PRD mode.

`/build` runs only against a **Locked** PRD — the requirements are frozen before any slice
reaches `/rplan`. The approval is written by `/proto` into **two** artifacts —
...
/build won't hand unfrozen scope to /rplan.
Lock it first:  /proto <E<n>|F<n> | prd-file.md>
...
(`/proto`) **before** build, so build itself never pauses to ask. A Locked PRD is by definition
...
until a full `/proto` locks the whole journey. Accepting two artifacts widens *where* the
...
tests account for it. **Tell `/rplan` to name the plan `<prd-slug>-slice<N>`** so it writes a
slice-distinct `tasks/<prd-slug>-slice<N>-plan.md` (a multi-slice build keeps several plans in `tasks/`;
newest-wins selection would otherwise let a later step bind to the wrong slice's plan). `/rplan`
will research, build the plan, and populate its Evidence Ledger. Record that exact plan path in the state
manifest's `artifact` and **pass it explicitly to `/rplan-review` (step 2), `/approved` (step 3), and
`/qa` (step 4)**. **Do not wait for approval** — read the resulting plan and its Blocking Set yourself.

### 2. `/rplan-review` — *only if needed*
Run the `rplan-review` skill when any of these hold; otherwise skip straight to step 3:
- `/rplan` leaves any blocking item unresolved (Blocking Set non-empty), **or**
...
`*-plan.md`. `/rplan-review` hardens the plan (criteria → test commands, domain-expert pass).
...
slice hand-rolled its own version), that is a **blocking finding — same bar as a `/reviewer`
...
diff (e.g. `/approved` had nothing committable, or left work uncommitted) means both
`/reviewer` and the step-5.5 security audit would review *nothing* and could report clean — a
false green. On a code-bearing slice an empty diff is a **gate failure**: commit the slice's work (or fix why
`/approved` didn't) and re-run — never accept a clean review of an empty diff.
...
1. **Implementation-level** (default — missing auth guard, unvalidated input, string-concat SQL,
   hardcoded secret, missing ownership / tenant check in a handler): fix via `/approved`
...
2. **Design-level** (the flaw is in the plan, not the code — no tenant-isolation model, wrong auth
   ...
   step 1 `/rplan`**, carrying the finding in as an explicit requirement/assumption, then
   `/rplan-review` (the slice is now HIGH-risk) → `/approved`.
   **If the fix reshapes the UI** (changes a screen, field, or flow the user sees — e.g. drops a
   ...
   stale: re-render it autonomously with `/proto --slice=N`, update
...
After any route, **re-run the tail** — `/qa` → `/reviewer` → this audit —
...
`/reviewer` has no blocking findings, **and** — if the slice was security-relevant — the
step-5.5 security audit is clean, **and** — if the slice had a `/proto` handoff — the
...
`/proto` handoff). The ABSOLUTE NO-GOS and the loop guard (~3 same-failure strikes → `BLOCKED:`)
still apply.
...
PLAN-mode analogue of what `/pickup` does on the PRD side. Build owns the terminal `Shipped`
flip (P6 / Completion Output), never here.

### P1. `/rplan` — plan the item (only if no plan yet)
...
build-driven (`/build` owns the resume state)** so it skips its own manual-chain state seed
(rplan Step 7 honors any build-driven invocation, PRD slice or PLAN item). `/rplan` normally
...
- **UI plan** → the design-system-reuse check (P5) is **always** on. **Additionally**, if the plan touches
  **>1 user-facing screen OR introduces a new visible state** (a new route/page, or a Gherkin scenario for a
  ...
  screen(s) runs after `/approved` (P3.5). Otherwise (single screen, no new state) the reuse
  check alone suffices; log `UI: reuse-check only`.

### P2. `/rplan-review` — only if needed
...
`/qa`'s Playwright path: generate/run a Playwright script that navigates each touched screen and
...
`UI-GLANCE: <k> screen(s) shot → tasks/build-<slug>-shots/`. (Why a post-`/approved` glance and
not a pre-build `/proto`: `/proto` is PRD-bound — it hard-stops without a PRD and
...
### P4. `/qa` — test against the plan's criteria
Invoke `qa` (pass this plan's path). It runs the plan's acceptance criteria as real tests; every criterion
must pass. **In the qa invocation, include the literal directive line `BUILD-DRIVEN: /build owns
...
### P5. `/reviewer` — fresh-context review + design-system-reuse check
**First confirm the item actually committed** — `git diff BASE..HEAD` must be non-empty (an empty committed
diff means reviewer would review nothing — a false green; commit the work or fix why `/approved`
...
```bash
# Detect the design system the same way /proto GATE 2 does:
...
same shallowest-skill routing (implementation → `/approved`; design → re-run `/rplan`
on the existing plan carrying the finding as an explicit requirement, then `/rplan-review` →
`/approved` — P1's "skip if a plan exists" does not apply to a deliberate security re-plan;
dependency-CVE → bump). Re-run the tail (P4 → P5 → this) until clean. Never fake-green a security hole.

### P6. Done
The item is "done" only when `/qa` is fully green **and** `/reviewer` is clean
(including the reuse check) **and** — if security-relevant — the P5.5 audit is clean. Then continue to the
FINAL GATE (e2e) and FINAL GATE 2 (`/wrap --heal`); the `Shipped` flip happens in Completion
...
- **Single-slice PRD:** report `⚠ NO E2E SUITE FOUND — slice-level /qa passed, but no
  end-to-end coverage exists.` and proceed (one slice's `/qa` is sufficient coverage).
- **PLAN mode (single plan-track item):** same as a single-slice PRD — report `⚠ NO E2E SUITE FOUND — plan
  /qa passed, but no end-to-end coverage exists.` and proceed (the item's `/qa` is
  sufficient coverage). If an e2e suite **does** exist, run it as above.

---

## FINAL GATE 2: Converge to clean (`/wrap --heal`)
...
`/wrap --heal` runs the full DoD gate (build, tests, coverage ≥80%, dep-CVE, secrets,
migration pairing, SonarQube) and, on any failure, **auto-heals instead of stopping** — routing each
failing gate to the shallowest skill (`/approved`, `/qa`, `sonar-fix-issue`,
...
The heal routes reuse the skills `/build` already drives, so no behavior is re-implemented
...
- **PRD-path launch** (the studio board and hand-typed `/build tasks/prd-<slug>.md`) — reverse-map:
...
automation can tell the two apart), written only once FINAL GATE 2 returned **Clean** — `/qa`
green, `/reviewer` clean (incl. the reuse check), any security audit clean-or-`n/a`,
...
- **No questions.** The only hard stops are: missing PRD (Gate 1), an **unlocked PRD** (Gate 1.5),
  ...
  not a prompt — resolve it by running `/proto` first, never by asking the user mid-build.
...
- **PLAN mode: the plan/item is the source of truth, and it's one unit.** Scope = the roadmap item's
  ...
  is **not** a stop — PLAN mode runs `/rplan` to create it.
...
- **Promote, don't re-invent.** When a `/proto` handoff exists (`tasks/proto-<slug>/notes.md`),
...
- **Converge before done, always.** After e2e, FINAL GATE 2 runs `/wrap --heal` — the
