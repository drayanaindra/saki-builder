...
7. **Sync the team baseline** (only if you edited the repo's `memory/`):
   ...
   - Team-baseline edits go through review: run `/sync`, which commits them on a branch and opens (or points you at) an MR — **never a direct push to `main`**. That MR is the governance gate that keeps the shared baseline curated.
