...
**Phase 1 — Setup**
1. Read MANIFEST (if resuming) or create new via `saki orchestrate`
2. Confirm output directory: `.saki/.output/features/{feature_id}/`
...
2. Output: `review_report.json` in `.saki/.output/features/{id}/`
...
1. Read the feature plan. Extract the `Test` field from each step in the Steps table — these are the test specs, not the implementation.
2. For each test group in plan:
   a. Spawn `tester` agent with: the plan's `Test` field entries as the test specifications. Instruct: "Write tests that verify the plan's spec. Do NOT derive tests from what the implementation does — derive them from what the plan says it should do."
...
```
.saki/.ephemeral/compaction-state.json
  status: "ok" → proceed
