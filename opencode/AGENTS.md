...
For non-trivial tasks: use `/rplan` before implementing (research → plan → readiness gate).
...
> [alternative]
> /wrap (if the task touched git — converge to clean)
> /retro (if the session was substantial)
```
...
---
*Detailed references (on demand): `config/docs/execution-protocol-detail.md`, `config/docs/xp-principles.md`, the `/clean-code` skill. Optional local gates (SonarQube pre-merge, etc.) are project-configured, not part of this core.*
