...
| **G3 — Foundations spec** ⟵ **human gate** | `foundations.md` + seed `design.md`: **stack · design-system approach · architecture (one load-bearing decision) · initial schema (entities+relations, shape altitude)** — each a cited decision + rejected alternative. **Stack = house default for frontend+DB (Next.js + Tailwind + shadcn/ui + Postgres), auto-applied. HARD RULE: `frontend/` + `backend/` are always separate top-level folders — no unified full-stack app, even for a simple MVP.** The backend *language* is **always prompted** (Go / Rust / Python / TypeScript-Node) — unless the prompt or a project file already specifies it, in which case honor that requirement and skip the prompt. | prd §7 Decision-Log discipline; `docs/modular-architecture.md`, `docs/ddd-patterns.md` |
...
3. **House-default stack, mandatory frontend/backend split** — frontend + DB are auto-applied defaults: **Next.js + Tailwind + shadcn/ui + Postgres** (matches proto GATE 2's shadcn/Tailwind escape hatch, so `/proto` recognizes the scaffold). **HARD RULE: every product is laid out as separate top-level `frontend/` and `backend/` folders from scratch — there is no unified full-stack app, even for a simple MVP.** The `backend/` service always exists; its **language is always prompted** (**Go / Rust / Python / TypeScript-Node**) — **unless the prompt or a project file already specifies the language/stack**, in which case that stated requirement is honored and the prompt is skipped. No silent default. G1/the vision may override the frontend/DB defaults per project. *(Refined 2026-07-09 — frontend/backend separation is now a non-negotiable hard rule overriding the earlier "full-stack Next.js → `none` backend" default; and the backend language is always prompted rather than defaulted, honoring any language already stated in the prompt/a file.)*
