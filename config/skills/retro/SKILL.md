...
3. **Write to memory**:
   ...
   - Format:
     ...
     ```
   - **Tag the stack** on any learning that involves a specific framework, language, or tool. Add `[stack: react]`, `[stack: python]`, `[stack: go]`, `[stack: ios]`, `[stack: mcp]`, or `[stack: ai]` at the end of the line. This lets `/saketek:reflect` route it to the right topic file without re-reading the session.

4. **Check for promotion candidates**:
   - If a learning appears 3+ times across sessions → flag for `/saketek:reflect`
