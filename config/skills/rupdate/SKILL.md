---
name: rupdate
description: (Legacy, owner symlink-install only) Pull latest skills/patterns/config from the remote saki-builder repo checkout. For the plugin, use /saketek:update instead.
---

# Update saki-builder from Remote (legacy symlink pull — plugin users: /saketek:update)
...
- After `/saketek:sync` runs on another machine and you want the latest on this one
- First thing in a new session on a machine that hasn't pulled recently

## Related

- `/saketek:sync` — push your learnings to remote (opposite direction)
