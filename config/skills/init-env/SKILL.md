...
> 2. **Do not port a provider-specific `model:` line.** Model identifiers are host-specific and a
>    wrong id breaks the agent. Omit `model` and let
...
7. **Create the `qa` agent**:
   ...
   **Where Steps 5–7 write, and in what shape:**
   ...
   - **`both`** → write both directories. The bodies are identical; only host-specific frontmatter
     differs. Model selection stays with each host.
