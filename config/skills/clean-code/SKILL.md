...
- Coverage **≥ 80%** (SonarQube new-code) — `/qa` enforces a local **≥ 70%** floor first (`COVERAGE_MIN`), so you clear coverage *before* the gate, not at it
- Duplicated lines **< 3%**
...
- Coverage: `/qa`'s **Coverage Gate** enforces a local floor (`COVERAGE_MIN`, **default 70%**) before push — it also gates the *changed files* to mirror SonarQube's new-code model; SonarQube's ≥80% new-code condition is the final check. Write tests with the code (TDD per `xp-principles.md`).
