...
- Coverage **≥ 80%** (SonarQube new-code) — `/saketek:qa` enforces the same **≥ 80% NON-NEGOTIABLE** floor locally first (`COVERAGE_MIN`, clamped — no lowering, no spike/skip bypass), so you clear coverage *before* the gate, not at it
- Duplicated lines **< 3%**
