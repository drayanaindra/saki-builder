...
**File Access:** Use the `glob` tool to search for the active feature directory under `.saki/.output/features/`. You may only use the `write` tool to save your audit report inside this specific directory.
...
4. **Generate Report:**
   - Write the results and the calculated health score to `.saki/.output/features/<active-feature-dir>/health-report.md` using the `write` tool.
