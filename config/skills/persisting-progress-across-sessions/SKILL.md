...
ls .saki/.output/features/
```

Look for a `MANIFEST.yaml` with `status: running` or `status: paused`. Read it:

```bash
cat .saki/.output/features/{feature_id}/MANIFEST.yaml
```
...
cat .saki/.output/features/{feature_id}/07-lead-architecture-plan.json

# Phase 8 output (implementation)
cat .saki/.output/features/{feature_id}/08-developer-{task_id}.json

# Any reviewer reports
cat .saki/.output/features/{feature_id}/09-reviewer-report.json
```
