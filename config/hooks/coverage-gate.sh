...
# is_push_to_main COMMAND → 0 if the command is a git push targeting main/master.
is_push_to_main() {
	local cmd="$1"
	echo "$cmd" | grep -qE 'git\s+push\b' || return 1
	echo "$cmd" | grep -qE '\b(main|master)\b' || return 1
	return 0
}
...
main() {
	local command="${CLAUDE_TOOL_INPUT_COMMAND:-}"
	...
	if [ -z "$file" ]; then
		if [ "$strict" = "1" ]; then
			...
			echo "coverage/lcov.info, coverage.out (walking up from $PWD)."
			return 1
		fi
