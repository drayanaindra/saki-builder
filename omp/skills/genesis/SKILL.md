...
- **Design-system approach — run Part 0 of the Design System Contract**
  (`skill://saki-builder-runtime/docs/design-system-contract.md`). This is the *one* time it runs.
...
G4 — Scaffold checklist (run these, then continue with /saki-builder:init-env):
  ...
  Claude dev env:
    ...
    [ ] design engine: ~/.omp/agent/hooks/design-engine-setup.sh record --engine native
```
