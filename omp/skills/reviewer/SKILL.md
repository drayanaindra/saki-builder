...
```bash
ls .omp/agents/reviewer.md 2>/dev/null && echo "PROJECT" || echo "GLOBAL"
```

- **PROJECT** → use `.omp/agents/reviewer.md` as the review checklist (project-specific rules)
...
(`skill://saki-builder-runtime/memory/patterns.md` + `patterns-<topic>.md`) and your **personal overlay**
(`~/.omp/agent/memory/patterns-personal.md`).

```bash
for f in "skill://saki-builder-runtime/memory/patterns.md" "$HOME/.omp/memory/patterns-personal.md"; do
...
[If project reviewer exists: paste contents of .omp/agents/reviewer.md checklist]
...
Used when no `.omp/agents/reviewer.md` exists.
...
- [ ] Instrumentation: every §5 `event`-class metric names an event that is **actually emitted at its wiring point** — declared-but-never-fired, fired on an error/validation path, or double-emitted on retry is a finding
...
- **Resume manifest (best-effort):** if a manual-chain manifest exists, stamp `reviewer=done` on APPROVE or
  ...
  `skill://saki-builder-runtime/docs/manual-chain-resume.md`), then run its **Stamp** snippet. Absent or
