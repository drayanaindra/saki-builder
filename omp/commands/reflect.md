...
1. **Read all lesson sources**:
   - Current project: `.omp/memory/lessons-learned.md`
   - Team baseline (read-only reference): `skill://saki-builder-runtime/memory/patterns.md` (+ `patterns-<topic>.md`) — or the saki-builder repo's `memory/` if you have it checked out
   - Personal overlay: `~/.omp/agent/memory/patterns-personal.md`
   - Other projects: search `~/.omp/agent/projects/*/memory/lessons-learned.md`
...
3. **Promote confirmed patterns — route by AUDIENCE first, then scope.**
   ...
   - **Personal overlay** `~/.omp/agent/memory/patterns-personal.md` — YOUR machine only, never pushed, injected every session by the core hook. Default home for anything project-specific-to-you, experimental, or < 3 confirmations. Write freely, no review.
   ...
   | Destination | When |
   |-------------|------|
   | `~/.omp/agent/memory/patterns-personal.md` (personal, instant) | project-specific-to-you, experimental, or < 3 confirmations — **default** |
...
5. **Write structured output to the chosen file**:
   ...
   **IMPORTANT:** Global memory files (`patterns.md`, `patterns-<topic>.md`) are public. Never write specific project names in the `source` field. Use generic descriptions:
   ...
   Project-local memory files (`<repo>/.omp/memory/patterns.md`) MAY use specific project context since they're scoped to that repo.
...
7. **Sync the team baseline** (only if you edited the repo's `memory/`):
   - Personal-overlay writes need NO sync — `~/.omp/agent/memory/patterns-personal.md` lives on your machine, never in the repo, and is injected each session.
