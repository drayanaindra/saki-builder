...
Create an execution plan following the template at `skill://rplan/template.md` (the sibling `template.md` in this skill's directory — glob for it if the variable isn't expanded).
...
- **Persona check:** after PRD ingestion, check if `.omp/personas/*.md` exists. If it does,
...
- **Schema change → load the expand-contract doctrine first.** If this task changes DB schema (any
  Migration Checklist row), read `skill://safe-migrations`
...
`/saki-builder:wrap`. Contract: `skill://saki-builder-runtime/docs/project-context-contract.md`.
...
- **Seed the manual-chain resume manifest** (skip for **any** `/saki-builder:build`-driven invocation —
  ...
  `skill://saki-builder-runtime/docs/manual-chain-resume.md` with `PLAN_FILE` = the plan just written and
