...
> /saketek:rplan add batch tracking       # namespaced, works on every teammate's machine
```
...
```
`/saketek:reflect` writes durable cross-person lessons to the team baseline (as an MR);
...
| 0.4 | Write the **config validator** (zero-dep Node, mirrors ed-harness `test/validate.js`): parse all `plugin.json`/`marketplace.json`/`hooks.json`; every `skills/*/SKILL.md` + `agents/*.md` has valid frontmatter; every `/saketek:<name>` reference resolves to a real skill/command/agent; version sync (`plugin.json` ↔ `CHANGELOG.md`); the rename/prefix invariant (no stray `claude-config`) | `test/validate.js`, `package.json` (add `"test": "node test/validate.js"`) | LOW | `node test/validate.js` exits 0 | Yes |
...
| 1.7 | Owner installs from LOCAL-path marketplace and verifies the loop | (no file) `/plugin marketplace add /Users/indrayana/claude-config` → `/plugin install saketek@saki-builder` | MED | `/saketek:rplan` runs; new session shows core rules present | Yes |

**Phase 1 gate:** a fresh session with the plugin installed can run `/saketek:rplan`, the core rules are in context, and `node test/validate.js` is green.
...
| 3.3 | Rewire `/saketek:reflect`: durable cross-person lessons → team baseline **as an MR** (not a direct push); personal/experimental → the private overlay. Replace the "everyone pushes `memory/`" model | `skills/reflect/SKILL.md` | MED | reflect writes to correct target per lesson class | Yes |
| 3.4 | Rewire `/saketek:sync`: stop pushing `memory/` for everyone; sync only owns team-baseline changes via MR; personal overlay never synced | `skills/sync/SKILL.md`, `sync.sh` | MED | sync no longer stages `~/.claude/memory/patterns-personal.md` | Yes |
...
| # | Action | Files (exact paths) | Risk | Test | Committable? |
|---|--------|---------------------|------|------|-------------|
| 4.1 | Add `/saketek:update` command (mirror ed-harness `commands/update.md`): report installed vs latest, give exact `/plugin` update commands, session-reload note | `commands/update.md` | LOW | `/saketek:update --check` | Yes |
...
| 4.5 | Push marketplace to GitLab (`solveed/saki-builder`); add to `extraKnownMarketplaces`; a teammate does a clean install to verify | GitLab remote + a teammate machine | MED | teammate `/plugin install` + `/saketek:rplan` works | Yes |
...
| **Teammate** | install/update via git-URL marketplace; use all `/saketek:*` commands; keep a private overlay; propose team-baseline lessons via MR | edit shipped plugin files locally (managed cache is read-only); push to `memory/` directly | plugin is read-only cache; team-baseline changes require MR | `/plugin marketplace add <gitlab-url>` + `/plugin install` |
...
```
config/skills/*/SKILL.md   → STAYS PUT; plugin.json "skills":["./config/skills"]  (invoked /saketek:<name>)
...
- **Step 0.1 (spike result) — RESOLVED → "don't resolve" branch taken:** spike proved invocation is namespaced (`/saki-spike:skill-a`), so run `bin/namespace-refs.js` (Step 2.2) to rewrite the ~380 refs to `/saketek:*`; the legacy symlink path loses bare cross-ref resolution (deprecated; plugin is primary).
...
1. ~~**[MED] Within-plugin bare-reference resolution.**~~ **RESOLVED (Phase-0 spike, 2026-07-02).** Built a throwaway 2-skill plugin, loaded it with `claude --plugin-dir`, and enumerated its commands: they appear **namespaced by `plugin.json` name** — `/saki-spike:skill-a`, `/saki-spike:skill-b`. Bare invocation is not the plugin model. **Verdict → namespace-all:** all ~380 internal cross-refs get rewritten to `/saketek:<name>` (Step 2.2 script required); the owner's legacy symlink path (bare) is therefore deprecated in favor of the local-marketplace install (already the plan's owner path). Validator enforces the prefix invariant.
...
- [ ] **Namespaced invocation works:** in a fresh session with the plugin installed, `/saketek:rplan` launches the rplan skill → `[manual]` (Playwright can't test the Claude REPL; verify by invocation)
...
- [ ] **Teammate clean install (Phase 4):** on a second machine, `/plugin marketplace add <gitlab-url>` + `/plugin install saketek@saki-builder` + `/saketek:rplan` succeeds → `[manual]`
...
- **2026-07-02 — Phase 1 COMPLETE.** Verified end-to-end via a real `claude plugin install` from the local marketplace:
  - **Skills (46)** load namespaced `/saketek:*` (inventory-confirmed). The 27 nested library skills correctly do NOT register as slash commands (they're `user-invocable:false` gateway docs — Phase 2 path-fix).
...
- **2026-07-02 — Phase 2 COMPLETE** (v0.2.0). Verified via real install: 46 skills, 3 hook events, ~2,422 always-on tok.
  - **319 internal refs namespaced** → `/saketek:*` (`bin/namespace-refs.js`, guarded regex; externals left bare). Validator now resolves every ref.
...
- **2026-07-02 — Phase 4 COMPLETE (code)** (v0.4.0). Verified via real install: 47 skills (incl. `update`), 3 hook events.
  - **`/saketek:update`** skill + **`check-plugin-update.js`** SessionStart nudge (pull-based, fail-open).
...
3. Teammates: `/plugin marketplace add https://gitlab.solveeducation.org/solveed/saki-builder.git` + `/plugin install saketek@saki-builder`.
