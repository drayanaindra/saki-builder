...
**A capability claim is a blocking item like any other — it needs a citation.** Probe before claiming absence: deferred tool (`ToolSearch`) → CLI on PATH (`command -v gh`) → installable (`brew`/`npx`) → env present (`[ -n "$VAR" ]` — test presence, never print the value). Cite the probe that failed — `BLOCKED: N8N_API_KEY unset ([ -n "$N8N_API_KEY" ] → empty)`, never "I don't have X". **Never probe around a refusal:** a denied permission, a missing credential, and interactive auth (`gcloud auth login`) are genuine human handoffs, not obstacles to route around.
